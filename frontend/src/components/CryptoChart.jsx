...
// Tooltip personalizat pentru afișarea informațiilor detaliate
const CustomTooltip = ({ active, payload, label, color }) => {
  if (active && payload && payload.length) {
    ...
    try {
      ...
      if (isNaN(dateObj)) {
        displayDate = label; // Folosim valoarea originală dacă nu putem converti
      } else {
        // Verificăm formatul în funcție de tipul graficului (intraday sau nu)
    ...
    // Adăugăm informații OHLC și volum dacă sunt disponibile
      ...
      <div style={{
        ...
        {hasOHLC && (
          <div style={{ marginTop: '4px', fontSize: '0.9em' }}>
            <p style={{ margin: '2px 0' }}>Open: ${payload[0].payload.open.toFixed(2)}</p>
            <p style={{ margin: '2px 0' }}>High: ${payload[0].payload.high.toFixed(2)}</p>
            <p style={{ margin: '2px 0' }}>Low: ${payload[0].payload.low.toFixed(2)}</p>
            {payload[0].payload.volume && (
              <p style={{ margin: '2px 0' }}>Volume: {payload[0].payload.volume.toLocaleString()}</p>
            )}
          </div>
        )}
      </div>
...
const CryptoChart = ({ data, color = '#F7931A', isIntraday = false }) => {
  ...
  // Determină valorile minime și maxime pentru axa Y cu o marjă adecvată
  // Folosim o marjă puțin mai mare pentru crypto deoarece volatilitatea este mai mare
  const prices = validData.map(item => Number(item.price));
  const minPrice = Math.min(...prices) * 0.995; // 0.5% mai jos
  const maxPrice = Math.max(...prices) * 1.005; // 0.5% mai sus
  ...
  // Formatează eticheta pentru axa X
  ...
  const formatYAxis = (value) => {
    ...
    if (value < 10) {
      return `$${value.toFixed(3)}`; // Trei zecimale pentru valori foarte mici (majoritatea cryptomonedelor)
    } else if (value < 1000) {
      return `$${value.toFixed(2)}`; // Două zecimale pentru valori medii
    } else {
    ...
    <ResponsiveContainer width="100%" height={400}>
      <LineChart data={formattedData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
        ...
        <YAxis 
          ...
          tick={{ fontSize: 12 }}
          // Algoritmul pentru a determina numărul optim de intervale pe axa Y
          ticks={[
            minPrice, 
        ...
        <Line 
          type="monotone" // Linie cu interpolare curbă pentru un aspect mai plăcut 
          dataKey="price" 
          stroke={color} 
          dot={false} // Fără puncte pe linie pentru a evita aglomerarea
          strokeWidth={2} // Linie mai groasă pentru cryptomonede
